...
internal class DiscountRule4 : RuleBase
{
    ...
    public DiscountRule4 (string targetTag, decimal discountAmount)
    {
        TargetTag = targetTag;
    ...
    public override IEnumerable<Discount> Process (CartContext cart)
    {
        ...
        foreach (var sku in cart.PurchasedItems.Where(pi => pi.Tags.Contains(TargetTag)).Select(pi => pi.SKU).Distinct())
        {
            matched.Clear();

            foreach (var p in cart.PurchasedItems.Where(pi => pi.SKU == sku))
            {
                matched.Add(p);
                if (matched.Count == 2)
                {
